...
badguy_image = pygame.image.load("파이썬으로 시작하는 코딩/images/badguy.png").convert()
badguy_image.set_colorkey((0,0,0))
fighter_image = pygame.image.load("파이썬으로 시작하는 코딩/images/fighter.png").convert()
fighter_image.set_colorkey((255,255,255))
missile_image = pygame.image.load("파이썬으로 시작하는 코딩/images/missile.png").convert()
missile_image.set_colorkey((255,255,255))
GAME_OVER = pygame.image.load("파이썬으로 시작하는 코딩/images/gameover.png").convert()
score = 0
shots = 0
hits = 0
misses = 0
...
class Fighter:
    def __init__(self):
        self.x = 320
    def move(self):
        if preesed_keys[K_LEFT] and self.x > 0:
            self.x -= 3
        if preesed_keys[K_RIGHT] and self.x < 540:
            self.x += 3
    def draw(self):
